...
#' @description Function to Duplicate deal on pipedrive.
...
#' @export
#' @examples \donttest{
#' deals.duplicate(id='e.g.',api_token='token',company_domain='exp')
#' }
deals.duplicate <- function(id, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/deals/{id}/duplicate?'
bodyList <- list(id=id)
