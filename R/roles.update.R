...
#' @description Function to Update role details on pipedrive.
...
#' @export
#' @examples \donttest{
#' roles.update(id='e.g.',api_token='token',company_domain='exp')
#' }
roles.update <- function(id, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/roles/{id}?'
bodyList <- list(id=id)
