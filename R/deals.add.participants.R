...
#' @description Function to Add a participant to a deal on pipedrive.
...
#' @export
#' @examples \donttest{
#' deals.add.participants(id='e.g.',person_id='e.g.',api_token='token',company_domain='exp')
#' }
deals.add.participants <- function(id, person_id, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/deals/{id}/participants?'
bodyList <- list(id=id,person_id=person_id)
