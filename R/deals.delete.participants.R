...
#' @description Function to Delete a participant from a deal on pipedrive.
...
#' }
deals.delete.participants <- function(id, deal_participant_id, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/deals/{id}/participants/{deal_participant_id}?'
url <- sub('{company_domain}',company_domain, url, fixed = TRUE)
url <- paste0(url, 'api_token={api_token}')
url <- sub('{api_token}',api_token, url, fixed = TRUE)
url <- sub('{id}',id, url, fixed = TRUE)
url <- sub('{deal_participant_id}',deal_participant_id, url, fixed = TRUE)
r <- httr::DELETE(url)
