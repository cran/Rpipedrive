...
#' @description Function to Get deals where a product is attached to from pipedrive.
...
#' }
products.get.deals <- function(id, start=NULL, limit=NULL, status=NULL, api_token=NULL, company_domain='api'){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/products/{id}/deals?'
bodyList <- list(id=id,start=start,limit=limit,status=status)
