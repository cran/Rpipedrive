...
#' @description Function to Merge two persons on pipedrive.
...
#' @export
#' @examples \donttest{
#' persons.update.merge(id='e.g.',merge_with_id='e.g.',api_token='token',company_domain='exp')
#' }
persons.update.merge <- function(id, merge_with_id, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/persons/{id}/merge?'
bodyList <- list(id=id,merge_with_id=merge_with_id)
