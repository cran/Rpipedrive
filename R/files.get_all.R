#' @title Get all files
#' @description Function to Get all files from pipedrive.
...
#' @export
#' @examples \donttest{
#' files.get_all(api_token='token',company_domain='exp')
#' }
files.get_all <- function(start=NULL, limit=NULL, include_deleted_files=NULL, sort=NULL, api_token=NULL, company_domain='api'){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/files?'
bodyList <- list(start=start,limit=limit,include_deleted_files=include_deleted_files,sort=sort)
