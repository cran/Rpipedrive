#' @title Update permission set details
#' @description Function to Update permission set details on pipedrive.
...
#' }
permissionsets.update <- function(id, contents, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/permissionSets/{id}?'
bodyList <- list(id=id,contents=contents)
