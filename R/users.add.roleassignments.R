...
#' @description Function to Add role assignment on pipedrive.
...
#' @export
#' @examples \donttest{
#' users.add.roleassignments(id='e.g.',role_id='e.g.',api_token='token',company_domain='exp')
#' }
users.add.roleassignments <- function(id, role_id, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/users/{id}/roleAssignments?'
bodyList <- list(id=id,role_id=role_id)
