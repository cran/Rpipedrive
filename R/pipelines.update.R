...
#' @description Function to Edit a pipeline on pipedrive.
...
#' }
pipelines.update <- function(id, name=NULL, deal_probability=NULL, order_nr=NULL, active=NULL, api_token=NULL, company_domain='api', return_type = c('complete','boolean')){
api_token <- check_api_token_(api_token)
url <- 'https://{company_domain}.pipedrive.com/v1/pipelines/{id}?'
bodyList <- list(id=id,name=name,deal_probability=deal_probability,order_nr=order_nr,active=active)
